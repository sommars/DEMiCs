...
//------------------------------------------------------------------------------
void supportSet::allocSupp(dataSet& Data, int level, int num, double* lifting){
	int cnt = 0;
	// cout << "<< allocSupp >>\n";
	row = Data.Dim;
	...
	for(size_t j = 0; j < col; j++){
		...
		for(size_t i = 0; i < row; i++){
			supMat_in(
				i, cnt, Data.support_out(Data.termStart[level] + j, i) 
					- Data.support_out(Data.termStart[level] + num, i)
			);
		}
		costVec[cnt] = lifting[Data.termStart[level] + j] - lifting[Data.termStart[level] + num];
		cnt++;
		}
	}
	col--;
...
//------------------------------------------------------------------------------
void simplex::allocateAndIni(dataSet& Data, int* ori_firIdx, int seedNum, int ori_output){
	cout << "<< allocAndIniSimplex >>\n";
	int i, j, k, cnt;
	...
	if(output){
		...
		cout << "* Lifting values for elements in each support set" << endl << endl;
		cnt = 0;
		for(i = 0; i < supN; i++){
			cout << "S";
			cout.width(3);
...
//------------------------------------------------------------------------------
void simplex::elimArt(int depth, int preNbN, int termS, int reTermS, int& iter){
...
int simplex::reducedCost(int& pivInIdx, int& sub_pivInIdx, double& redCost){
	...
	for(j = 0; j < nbN - Dim; j++){
		...
		*/
		...
		(redVec[tmp_non_basisIdx] = (tmp_redCost = Supp[level][idx2].redVal(d_sol, idx, ii)));
...
//------------------------------------------------------------------------------
int simplex::reducedCost_Bland(int& pivInIdx, int& sub_pivInIdx, double& redCost){
...
//------------------------------------------------------------------------------
int simplex::reducedCost_tab_p1(int& pivInIdx, int& sub_pivInIdx, double& redCost){
...
//------------------------------------------------------------------------------
int simplex::reducedCost_tab(int& pivInIdx, int& sub_pivInIdx, double& redCost){
...
//------------------------------------------------------------------------------
int simplex::reducedCost_p1(int& pivInIdx, int& sub_pivInIdx, double& redCost){
	...
	for(j = 0; j < nbN - Dim; j++){
		...
		*/
		...
		(redVec[tmp_non_basisIdx] = (tmp_redCost = aux_cvec[tmp_non_basisIdx] - val));
...
//------------------------------------------------------------------------------
int simplex::reducedCost_mFst(int& pivInIdx, int& sub_pivInIdx, int pivOutIdx, 
...
//------------------------------------------------------------------------------
int simplex::reducedCost_iFst(int& pivInIdx, int& sub_pivInIdx, int pivOutIdx, 
						int sub_pivOutIdx, double& redCost, 
	...
	for(j = 0; j < length; j++){
		...
		*/
		...
		(redVec[non_basisIdx] =(tmp_redCost = Supp[level][idx2].redVal(d_sol, idx, ii)));
...
//------------------------------------------------------------------------------
void simplex::extend_nbIdx(int cIdx, int pre_pivInIdx, int pre_pivOutIdx, 
...
//------------------------------------------------------------------------------
int simplex::extend_nbIdx_comp(int& non_basisIdx, int cIdx, 
						 int pre_pivInIdx, int pre_pivOutIdx, 
						 int pre_length, int reTermS, int& cnt){
...
//------------------------------------------------------------------------------
inline void simplex::getIdx(int& level, int& idx, int& idx2, int& ii, int d_nbIdx){
...
//------------------------------------------------------------------------------
int simplex::ratioTest(int redFlag, int pivInIdx, int sub_pivInIdx, int& pivOutIdx, 
...
//------------------------------------------------------------------------------
int simplex::ratioTest_artFst(int redFlag, int pivInIdx, int sub_pivInIdx, int& pivOutIdx, 
						int& sub_pivOutIdx, double& theta){
...
//------------------------------------------------------------------------------
int simplex::ratioTest_art(int redFlag, int pivInIdx, int sub_pivInIdx, int& pivOutIdx, 
				 int& sub_pivOutIdx, double& theta){
...
//------------------------------------------------------------------------------
int simplex::ratioTest_art_Bland(int redFlag, int pivInIdx, int sub_pivInIdx, int& pivOutIdx, 
				 int& sub_pivOutIdx, double& theta){
...
//------------------------------------------------------------------------------
void simplex::createNewBandN_tab(int pivInIdx, int sub_pivInIdx, 
	...
	for(i = 0; i < nfN; i++){
		...
		if(nfPos != sub_pivOutIdx) p_sol[basisIdx[nfPos]] +=	theta * dir[nfPos];
		else p_sol[basisIdx[nfPos]] = 0;
...
//------------------------------------------------------------------------------
void simplex::createNewBandN_p1(int pivInIdx, int sub_pivInIdx, 
				int pivOutIdx, int sub_pivOutIdx, double theta, 
...
//------------------------------------------------------------------------------
void simplex::createNewBandN(int pivInIdx, int sub_pivInIdx, 
					 int pivOutIdx, int sub_pivOutIdx, double theta, 
					 double redCost, int termS, int reTermS){
...
//------------------------------------------------------------------------------
void simplex::createNewBandN_iFst(int pivInIdx, int sub_pivInIdx, 
					int pivOutIdx, int sub_pivOutIdx, double theta, 
					double redCost, int termS, int reTermS){
	...
	for(i = 0; i < nfN; i++){
		...
		if(nfPos != sub_pivOutIdx){
			...
			p_sol[basisIdx[nfPos]] = pre_p_sol[basisIdx[nfPos]] + theta * dir[nfPos];
	...
	transMat[ii + i] = eye[ii + i] - tmp_transMat[i] * (dir[nfPos] / elem);
...
//------------------------------------------------------------------------------
void simplex::createNewBandN_mFst(int pivInIdx, int sub_pivInIdx, 
					int pivOutIdx, int sub_pivOutIdx, double theta, 
					double redCost, int termS, int reTermS){
...
//------------------------------------------------------------------------------
void simplex::createNewBandN_art(int pivInIdx, int sub_pivInIdx, 
...
int simplex::tSolLP(int& iter, int mode){
	...
	while(true){
		...
		flag = ratioTest(redFlag, pivInIdx, sub_pivInIdx, pivOutIdx, sub_pivOutIdx, theta);
	...
	break;
			...
			flag = ratioTest_art(redFlag, pivInIdx, sub_pivInIdx, pivOutIdx, sub_pivOutIdx, theta);
...
int simplex::checkFrIdx(){
	...
	if(flag == PIVOT_IN){
		...
		if(redCost > PLUSZERO) redFlag = NEGTHETA;
		else redFlag = POSTHETA;
...
int simplex::fSolLP(int termS, int reTermS, int& iter){
	...
	flag = solLP_art_Bland(pivInIdx, sub_pivInIdx, pivOutIdx, sub_pivOutIdx,
						 redFlag, theta, redCost, termS, reTermS, iter_2);
...
//------------------------------------------------------------------------------
void simplex::cal_redVec(int termS, int reTermS, int fst_pivInIdx, data** cur){
	...
	for(i = 0; i < termS; i++){
		...
		if(i != fst_pivInIdx){
			...
			(*cur)->redVec[cnt + reTermS] = fst_redVec[i] - fst_redVec[fst_pivInIdx];
			cnt++;
...
//------------------------------------------------------------------------------
void simplex::fstRed_candIdx(inifData& curInif, int** mCandIdx, 
...
//------------------------------------------------------------------------------
int simplex::solLP_art(int depth, int idx_one, int fst_pivIn, int preNbN, int termS, 
...
//------------------------------------------------------------------------------
int simplex::solLP_art_Bland(int pivInIdx, int sub_pivInIdx, 
					 int pivOutIdx, int sub_pivOutIdx,
	...
	while(true){
		...
		flag = ratioTest_art_Bland(redFlag, pivInIdx, sub_pivInIdx, pivOutIdx, sub_pivOutIdx, theta);
		...
		createNewBandN(pivInIdx, sub_pivInIdx, pivOutIdx, sub_pivOutIdx, theta, redCost, 
			 termS, reTermS);
...
//------------------------------------------------------------------------------
int simplex::solLP(int depth, int fst_pivInIdx, int fst_sub_pivInIdx, double fst_redCost,
...
//------------------------------------------------------------------------------
int simplex::solLP_Bland(int pivInIdx, int sub_pivInIdx, 
			 int pivOutIdx, int sub_pivOutIdx,
			 int redFlag, double theta, double redCost, 
			 int termS, int reTermS, int& iter){
...
//------------------------------------------------------------------------------
int simplex::initIter(int mode, int fst_pivInIdx, int fst_sub_pivInIdx, double fst_redCost,
	...
	case ICHECK:
		...
		if(flag == UNBOUNDED){
			...
			createNewBandN_iFst(fst_pivInIdx, fst_sub_pivInIdx, pivOutIdx, sub_pivOutIdx, 
				theta, redCost, termS, reTermS);
	...
	case MCHECK:
		...
		if(flag == UNBOUNDED){
			...
			createNewBandN_mFst(fst_pivInIdx, fst_sub_pivInIdx, pivOutIdx, sub_pivOutIdx, 
				theta, redCost, termS, reTermS);
