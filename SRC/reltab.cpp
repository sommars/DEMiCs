...
//------------------------------------------------------------------------------
void reltab::allocateAndIni(simplex& ori_Simplex, int** ori_firIdx, 
...
//------------------------------------------------------------------------------
void reltab::get_init_squData(int lab_a, int lab_b, int idx_a, int idx_b, 
	...
	srandom(4);
	for(size_t i = 0; i < constNum_a; i++){
		nbIdx[i] = reTermS_a + i; 
		val[i] = (double) rand() / (double) RAND_MAX;
	}

	for(size_t i = 0; i < constNum_b; i++){
	...
	for(size_t i = 0; i < Dim; i++){
		elem = 0;
		for(size_t j = 0; j < constNum_a; j++)
			elem += val[j] * Simplex->put_elem_supp(lab_a, idx_a, i, j);
		for(size_t j = 0; j < constNum_b; j++)
...
//------------------------------------------------------------------------------
void reltab::findAllFeasLPs_squ(int lab_a, int lab_b, int idx_a, int idx_b, 
				int colPos, int rowPos){
...
void reltab::info_p_sol(){
	cout << "<< p_sol >> " << endl;
	for(size_t i = 0; i < maxConst + Dim; i++)
		cout << p_sol[i] << " ";
  cout << endl << endl;
}
...
void reltab::info_allTable(){
	...
	for(size_t j = 0; j < termSumNum; j++){
		for(size_t i = 0; i < termSumNum; i++){
			cout << table_out(j, i) << " ";
			if(table_out(j, i) == UNBOUNDED) 
				unbNum++;
		}
		cout << endl;
	}
...
void reltab::info_table(){
	...
	for(size_t i = 0; i < termSumNum; i++){
		for(size_t k = 0; k < i; k++)
			cout << "  ";
		for(size_t j = i + 1; j < termSumNum; j++){
			if(table_out(i, j) == UNBOUNDED)
				u_cnt++;
			cout << table_out(i, j) << " ";
			t_cnt++;
		}
	...
	cout << "# Unb. LPs: " << u_cnt << endl;
	cout << "# Elem.: " << t_cnt << endl;
	cout << "Ratio: " << (double) u_cnt / (double) t_cnt << endl << endl;
}

//------------------------------------------------------------------------------
void reltab::makeTable(double& total_unbLP_tab){
	// JEFF START HERE!! Figure out what is going into their relation tables and 
	// how they're being built. If I can understand how that's happening,
	// everything else _should_ be doable.
