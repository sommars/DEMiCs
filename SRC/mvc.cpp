...
mvc::~mvc(void){
	if(trNeg){
		for(size_t i = 0; i < termSet[sp[0]]; i++)
			delete [] trNeg[i];
		delete [] trNeg;
		trNeg = NULL;
	}
...
void mvc::getMemory(int depth, int lvl, int length){
	...
	for(size_t i = 0; i < termSet[depth]; i++){
...
//------------------------------------------------------------------------------
int mvc::chooseSup(int depth, data* curNode, 
	...
	switch(curNode->artV){
		case 0:
			flag = findUnbDir(nextInif, curNode, iLv[depth + 1].rsp, iLv[depth].rsp, depth, false);
			break;
		case 1:
			flag = findUnbDir(nextInif, curNode, iLv[depth + 1].rsp, iLv[depth].rsp, depth, true);
			break;
	}
	return (flag);
}
...
//------------------------------------------------------------------------------
void mvc::fUpdateDirRed(inifData* curInif, inifData* nextInif, 
...
//------------------------------------------------------------------------------
void mvc::updateDirRed(inifData* curInif, inifData* nextInif, 
					 data* curNode, int* curRsp, int depth){
	int i, j, k;
...
//------------------------------------------------------------------------------
int mvc::findUnbDir(inifData* nextInif, data* curNode, 
...
//------------------------------------------------------------------------------
int mvc::checkDir(uData** corPtr, uData* tarPtr, 
...
//------------------------------------------------------------------------------
int mvc::checkDir_art(uData** corPtr, uData* tarPtr, 
					double* tar_dir, double tar_red, 
					int* nf_pos, int* basisIdx, int nfN){
	int i;
	int nfPos;
...
//------------------------------------------------------------------------------
void mvc::initLP(ftData& Data, int** negIdx, int depth, int idx, int& feaNum){
	...
	Data.cur->invB[negIdx[idx][j + 1] + k * Dim] *= -1;
		...
		cout << "Error: too much iterations at initLP\n\n";
		cout << "( " << idx << " ) \n\n";
...
// flag -- FNN (Node\A4\F2\BD\D0\CEϤ\B7\A4\C6, findeNextNode)
//			-- STOP (\A5롼\A5פ\F2\A4̤\B1\A4\EB)
	int polyDim, flag, sn;
	...
	while(1){
		...
		if(mFea[lvl] <	polyDim - lvl + 1 || lvl == polyDim){	// SLIDE
...
//------------------------------------------------------------------------------
void mvc::findUpNode(ftData* Data, ftData** pre, ftData** cur, int& lvl, 
				 int polyDim, int depth){

	
	while(1){
			
		if(mFea[lvl - 1] - mRepN[lvl - 1] - 1 < polyDim - lvl + 1){ // UP
...
//------------------------------------------------------------------------------
int mvc::mLP(ftData* Pre, ftData* Cur, ftData* Data, 
			 int* repIdx, int* feaIdx, int tarIdx, 
			 int* mRepN, int totalN, int depth, 
			 int& feaNum, int lvl, int length){
...
int mvc::checkBasis(data* target, int sub_sIdx){
	...
	if(target->rIdx[sub_sIdx - 1] >= 0) return (OPT);
...
int mvc::checkAnotherBasis(int repIdx, int dist, data** target){
	...
	if((*target)->rIdx[repIdx] >= 0) return (OPT);
	else return (CONTINUE);
...
int mvc::iCheck(int depth, data* parent, ftData& Data, inifData& curInif){
	...
	case true:
		...
		while(curr != NULL){
			...
			iLP_Art(parent, Data, depth, curr->supLab, fst_pivInIdx, sub_fst_pivInIdx, 
				preNbN, feaNum);
	...
	case false:
		...
		Simplex.fstRed_candIdx(curInif, &candIdx, fst_pivInIdx, sub_fst_pivInIdx);
		...
		while(curr != NULL){
			...
			iLP(parent, Data, depth, curr->supLab, fst_pivInIdx, sub_fst_pivInIdx, 
		preNbN, feaNum);
...
//------------------------------------------------------------------------------
void mvc::iLP(data* parent, ftData& Data, int depth, int idx_one, int fst_pivInIdx, 
	...
	if(repIdx != fst_pivInIdx){
		...
		if(flag == OPT){
			...
			cout << "Error: too much iterations at iLP\n";
...
//------------------------------------------------------------------------------
void mvc::iLP_Art(data* parent, ftData& Data, int depth, int idx_one, 
	...
	Data.iCopy(preNbN, lNfN, repIdx, termSet[sn], reTermS, 
			 candIdx, parent);
	Data.init_info();
	...
	flag = Simplex.solLP_art(depth, repIdx, fst_pivInIdx, preNbN, 
				 termSet[sn], reTermS, iter);
	...
	if(flag == OPT){
		...
		cout << "Error: too many iterations at iLP_Art\n";
