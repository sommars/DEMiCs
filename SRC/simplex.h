...
class simplex{
	private:
		...
		// reduced cost
		int reducedCost_tab_p1(int& enterIdx, int& sub_enterIdx, double& redCost);
		...
		int reducedCost_Bland(int& enterIdx, int& sub_enterIdx, double& redCost);
		...
		int extend_nbIdx_comp(int& non_basisIdx, int cIdx, int pre_pivInIdx,
			int pre_pivOutIdx, int pre_length, int reTermS, int& cnt);

		void getIdx(int& level, int& idx, int& idx2, int& ii, int d_nbIdx);

		// ratio test
		int ratioTest(int redFlag, int pivInIdx, int sub_pivInIdx, int& pivOutIdx, 
			int& sub_pivOutIdx, double& theta);

		int ratioTest_artFst(int redFlag, int pivInIdx, int sub_pivInIdx, int& pivOutIdx, 
			int& sub_pivOutIdx, double& theta);

		int ratioTest_art(int redFlag, int pivInIdx, int sub_pivInIdx, int& pivOutIdx, 
			int& sub_pivOutIdx, double& theta);

		int ratioTest_art_Bland(int redFlag, int pivInIdx, int sub_pivInIdx, int& pivOutIdx, 
			int& sub_pivOutIdx, double& theta);
		...
		void createNewBandN_tab(int pivInIdx, int sub_pivInIdx, 
			int pivOutIdx, int sub_pivOutIdx, double theta, double redCost);

		void createNewBandN_p1(int pivInIdx, int sub_pivInIdx, 
			int pivOutIdx, int sub_pivOutIdx, double theta, double redCost, int termS, 
			int reTermS);
	...
	public:
		...
		void allocateAndIni(dataSet& Data, int* ori_firIdx, int seedNum, int ori_output);
		...
		// For mLP
		int solLP(int depth, int fst_pivInIdx, int fst_sub_pivInIdx, double fst_redCost,
		...
		int initIter(int mode, int fst_pivInIdx, int fst_sub_pivInIdx, double fst_redCost,
			int& redFlag, int& pivInIdx, int& sub_pivInIdx, 
			int& pivOutIdx, int& sub_pivOutIdx, double& theta, double& redCost, 
			int termS, int reTermS, int preNbN);
